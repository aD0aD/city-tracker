...
用一张中国地图，记录并可视化你去过的城市。数据默认保存在浏览器本地（`localStorage`），不需要后端服务。
...
如果你想在仓库主页展示效果，建议把截图放到 `docs/` 目录，然后在这里引用，例如：

```markdown
![地图示例](docs/screenshot.png)
```
...
- **城市名称要匹配地图**：例如 `北京市` 而不是 `北京`（需要与地图数据里的行政区名称一致）。
- **地图数据来源**：项目可能需要从公网拉取地图 geoJSON；如果你所在网络无法访问相关域名，地图会加载失败（详见 `启动指南.md`）。

### 数据与隐私

- **默认存储位置**：浏览器 `localStorage`（仅当前浏览器/当前设备）
- **不会自动上传**：本项目不包含后端与账号系统，不会把数据发到服务器
- **清理方式**：清除站点数据/浏览器缓存会导致数据丢失（如需备份可自行实现导出）
